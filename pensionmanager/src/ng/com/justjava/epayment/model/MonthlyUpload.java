...
@Views({@View(members="uploadYear;month;monthlyFigure;holders"),
...
@Tabs({@Tab(properties="month,narration,status",filter=MultiValueFilter.class,
		...
		@Tab(name="MyUpload",properties="month,narration,status,paymentResponseCode,paymentResponseDescription",
		baseCondition = "${deleted}=0 AND ${corporate.id}=?",filter=LoginUserCorporateFilter.class)
})
@Entity
public class MonthlyUpload {
	...
	public boolean pay() throws RemoteException{
		...
		try {
			...
			if(response.getError() != null && "0".equalsIgnoreCase(response.getError().trim())){
				result = true;
	...
	public FundRequest getBulkFundRequest(){
        ...
        String terminalId = getPayingAccount()!=null?getPayingAccount().getTerminalId():" ";
		...
		for(PensionFundAdministrator payItem:pfas){
			...
			if(account != null){
				localAmount = Double.valueOf(format.format(payItem.getAmountSummation().doubleValue()));
				bulkAmount = bulkAmount + localAmount;
				String bankCode = (account!=null?account.getBank().getCode():"NOT AVAILABLE");
				String accountName = (account!=null?account.getName():"NOT AVAILABLE");
				String accountNumber = (account!=null?account.getNumber():"NOT AVAILABLE");
				System.out.println(" Amount for " +payItem.getName() + " is "+ localAmount+
						" the destination account code=="+ bankCode
						+ " and the account number =="+accountNumber);
				...
				item.setUniqueId(payItem.getId() + "_" + RandomStringUtils.randomAlphanumeric(10).toLowerCase());	
	...
	public boolean updateStatuseTranzact() {
		...
		boolean result = false;
		try {
			System.out.println(" About to call a webservice here for bulk query=====");
			...
			for (BulkItem item : response.getBulkItems().getBulkItem()) {
				...
				PaymentLog log = PaymentLog.getPaymentLogByUniqueId(item.getUniqueId());
				if(log != null){
	...
	public FundRequest getBulkFundRequest(String action){
        ...
        System.out.println("/n/n/n/n/n/n/n/n/n/n/n/n/n/n/n/n/n/n/n/n/n/n/n The parameters are ===="
        		...
        		+ "  the reference===" + trans.getReference() +" the action here =="+action);        
		...
		for(PensionFundAdministrator payItem:payItems){
			...
			if(account != null){
				...
				item.setAmount(localAmount);
				item.setBankCode(StringUtils.trim(bankCode));
				item.setNarration(payItem.getId() + "_" + corporate.getUniqueIdentifier() + "_"+month);
				//item.setUniqueId(payItem.getId() + "_" + RandomStringUtils.randomAlphanumeric(10).toLowerCase());	
				bulkItems.getBulkItem().add(item);
